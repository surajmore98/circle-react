...
import { Flex, Box, Button, IconButton, Input, Textarea, useColorMode, Image } from "@chakra-ui/react";
        ...
        <Flex p={4} background={bgColor} width={{ base: "19rem", md: "22rem"}}>
            <Flex flexDirection="column" justifyItems="center" width="full">
                ...
                <Flex justifyContent="space-between" py={2} px={0}>
                    <Flex gap="0.25rem">
                        <IconButton aria-label="image" color="cyan.400" bg="transparent" icon={<FiImage />} fontSize="1.5rem" onClick={clickHandler} borderRadius="full"/>
                        <EmojiInput action={emojiInputHandler} borderRadius="full"/>
                    </Flex>
                    <Flex gap="0.25rem">
                        { content && <Button colorScheme="cyan" p="0.5rem 1.75rem" borderRadius={0} variant="outline" onClick={clearContent}>Clear</Button>}
                        <Button colorScheme="cyan" p="0.5rem 1.75rem" borderRadius={0}  ml="auto" onClick={submitHandler}>Post</Button>
                    </Flex>   
                </Flex>
            </Flex>
