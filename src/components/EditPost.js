import { Avatar, Box, Button, Flex, Heading, IconButton, Image, Input, Popover,
...
export const EditPost = ({data}) => {
    ...
    return(
        <Popover>
            ...
            <>
                <PopoverTrigger>
                    <IconButton aria-label="open" color="gray.400" bg={bgColor} icon={<MdEdit />} fontSize="1rem" borderRadius="full" minWidth={6} height={6} />
                </PopoverTrigger>
                <PopoverContent width="30rem" background={bgColor}>
                    <PopoverCloseButton />
                    ...
                    <PopoverBody>
                        <Flex gap="0.25rem" p="1rem" minWidth="25rem" width="full" background={bgColor}>
                            <Flex>
                                <Avatar name={username} src={profileImage} display="flex" size="lg"/>
                            </Flex>
                            <Flex flexDirection="column" justifyItems="center" width="full">
                                ...
                                <Flex justifyContent="space-between" py={2} px={0}>
                                    <Flex>
                                        <IconButton aria-label="image" color={themeColor} bg="transparent" icon={<FiImage />} fontSize="1.5rem" onClick={clickHandler}/>
                                        <EmojiInput action={emojiInputHandler} borderRadius={0}/>
                                    </Flex>
                                    <Flex gap="0.25rem">
                                        <Button colorScheme={themeColor} p="0.5rem 2rem" borderRadius={0} variant="outline" onClick={onClose}>Close</Button>
                                        <Button colorScheme={themeColor} p="0.5rem 2rem" borderRadius={0}  ml="auto" onClick={async (e) => {
